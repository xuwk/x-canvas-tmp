xc.module.define("xc.createjs.Graphics", function(exports) {
  ...
  /**
   * Inner class used by the {{#crossLink "Graphics"}}{{/crossLink}} class.
  ...
  var Graphics = xc.class.create({
    ...
    beginFill: function(color) {
      ...
      this._fillInstructions = color ?
          [new Command(this._setProp, ["fillStyle", color], false), Graphics.fillCmd] : null;
      return this;
    },
     ...
     * @param {String | Number} [caps=0] Indicates the type of caps to use at the end of lines. One of butt, round, or square.
     ...
     * @param {Boolean} [ignoreScale=false] If true, the stroke will be drawn at the specified thickness regardless of
    ...
    drawRoundRectComplex: function(x, y, w, h, radiusTL, radiusTR, radiusBR, radiusBL) {
      ...
      this._activeInstructions.push(new Command(this._ctx.moveTo, [x + w - radiusTR, y]),
    ...
    drawPolyStar: function(x, y, radius, sides, pointSize, angle) {
      ...
      if (angle == null) { angle = 0; } else { angle /= 180 / Math.PI; }
      var a = Math.PI / sides;
  ...
  /**
   * Maps the familiar ActionScript <code>curveTo()</code> method to the functionally similar {{#crossLink "Graphics/quadraticCurveTo"}}{{/crossLink}} method.
   *
   * @method curveTo
   * @type {Function}
   */
  ...
  /**
   * Maps the familiar ActionScript <code>drawRect()</code> method to the functionally similar {{#crossLink "Graphics/rect"}}{{/crossLink}} method.
   *
   * @method drawRect
   * @type {Function}
   */
